...
import '../../../two_wheeler_insurance/two_wheeler_details/model/ResGetCityRto.dart';
import '../../../two_wheeler_insurance/two_wheeler_details/model/ResGetPolicyCities.dart';
...
import '../../../two_wheeler_insurance/two_wheeler_details/model/bike_tabs_model.dart';
...
class CarInsuranceController extends GetxController {
  ...
  Rx<TextEditingController> carNumberController = TextEditingController().obs;
  ...
  Future<void> onReady() async {
    ...
    dashboardController.fetchInsuranceSliderImages('2');
  ...
  Future<void> findVehicleDetails(vehicle_num) async {
    ...
    apiClientProvider
      ...
      if (success) {
        ...
        if(resVerifyVehicleDetails.data.vehicleCategory.toString() == '2WN'){
          Dialogs.showInfoDialog('No vehicle Found');
          return;
        }
        ...
        try {
          ...
          carInsuranceController.selectedCity.value =
              Utils.fetchCityFromAddress(
                      resVerifyVehicleDetails.data.permanentAddress)
                  .trim();
          carInsuranceController.selectedRTO.value =
              vehicle_num.toString().replaceAll('-', '').substring(0, 4);
          ...
          ;
          carInsuranceController.selectedVariant.value.name =
              resVerifyVehicleDetails.data.makerModel;
          carInsuranceController.selectedYear.value = resVerifyVehicleDetails
  ...
  Future<bool> fetchManufacturer({String city = ''}) async {
    ...
    modelList.clear();
    variantList.clear();
    await apiClientProvider.getVehicleCompanyList('', 1).then((response) async {
  ...
  Future<List<String>> fetchPolicyCityList({String city = ''}) async {
    ...
    print('--city--${jsonEncode(response.body)}');
    bool success =
        await apiClientProvider.checkStatus(response, wantToShowDialog: false);
    if (success) {
