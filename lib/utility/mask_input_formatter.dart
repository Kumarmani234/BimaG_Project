...
class CustomTextInputFormatter implements TextInputFormatter {
  ...
  String maskText(String text) {
    return CustomTextInputFormatter(
...
class CommaFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
      TextEditingValue oldValue, TextEditingValue newValue) {
    // Check if the new value is empty or invalid
    if (newValue.text.isEmpty || newValue.text == '.' || newValue.text == ',') {
      return newValue;
    }

    // Replace any commas in the new value with empty strings
    String sanitized = newValue.text.replaceAll(',', '');

    // Parse the sanitized value as a double
    double? value = double.tryParse(sanitized);

    // Check if the value is null (i.e., if the input is invalid)
    if (value == null) {
      return oldValue;
    }
    ...
    // Check if the formatted value is the same as the new value
    if (formatted == newValue.text) {
      return newValue;
    }

    // Return the new value with commas added
  ...
  String formatWithCommas(double value) {
    List<String> parts = value.toStringAsFixed(0).split('');
